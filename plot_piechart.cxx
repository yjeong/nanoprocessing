...
void set_piechart_Title(TPie *p_opt){
...
int main()
{
	...
	for(int ibin=22; ibin<34; ibin++){
		for (int inb=0; inb<3; inb++){
			Float_t vals[5] = {qcd[ibin][inb],ttbar[ibin][inb],wjets[ibin][inb],other[ibin][inb],sig1600[ibin][inb]};
			Int_t colors[5] = {kYellow-7,kAzure+7,kGreen+2,kGray+1,kRed};
			Int_t nvals = sizeof(vals)/sizeof(vals[0]);
			cpie[ibin][inb] = new TCanvas(Form("cpie_%i_%i",ibin,inb),Form("TPie test_%i_%i",ibin,inb),700,700);
			pie4[ibin][inb] = new TPie(Form("pie4_%i_%i", ibin,inb),Form("Pie with verbose labels_%i_%i",ibin,inb),nvals,vals,colors);
			set_piechart_Title(pie4[ibin][inb]);
			pie4[ibin][inb]->Draw("nol <");
			cpie[ibin][inb]->SaveAs(outputdir+Form("test_piechart_%i_%i.pdf",ibin,inb));
		}
	}
}
